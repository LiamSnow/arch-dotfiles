...
"""
Original: fsantand (https://gist.github.com/fsantand/846fbdd9ed2db5c89838b138a2e48ceb)
...
Usage:
./wireplumber_audio_switcher.py <Sinks|Sources> <Config File>
"""
...
def pipe_into_dmenu(output, config_file):
    output = subprocess.run(
        f"echo '{output}' | rofi -dmenu -markup-rows -config {config_file}",
        shell=True,
        encoding="utf-8",
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
...
sink = pipe_into_dmenu("\n".join(output[sys.argv[1]]), sys.argv[2])
